...
class HistoryLogListBuilder extends EntityListBuilder {
  ...
  public function __construct(
    ...
    // Get the entity from params which can be used to filter entity query.
    foreach (RestContentPusher::SUPPORTED_ENTITY_TYPES as $type) {
      $entity = $this->routeMatch->getParameter($type);
      if ($entity) {
        $this->entityIdFilter = $entity->id();
      }
    }
  }
  ...
  public function load() {
    ...
    if ($this->entityIdFilter) {
      $entity_query->condition('target_entity_id', $this->entityIdFilter);
    }
